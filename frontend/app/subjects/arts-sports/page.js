...
export const metadata = {
  title: 'Arts & Sports Subjects | CBC Senior School',
};

export default function ArtsSportsSubjectsPage() {
  const pathway = pathwayThemes['arts-sports'];
  const subjects = subjectsByPathway['arts-sports'];
    ...
    <main className="mx-auto max-w-7xl p-6 md:p-10">
      <header className={`rounded-3xl p-6 shadow-sm ring-1 md:p-8 ${pathway.softBg} ${pathway.cardRing}`}>
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className={`text-xs font-bold uppercase tracking-wide ${pathway.accentText}`}>Orange pathway</p>
            <h1 className="mt-3 text-4xl font-bold text-slate-950">Arts & Sports subjects</h1>
            <p className="mt-3 max-w-3xl text-slate-700">
              Creative arts, physical education, home science, and construction subjects for Grades 10-12 with portfolio and performance evidence.
            </p>
          </div>
          <Link className="text-sm font-semibold text-orange-700 hover:text-orange-900" href="/subjects">
            ← All subjects
          </Link>
        </div>
      </header>
        ...
        {subjects.map((subject) => (
          <article key={subject.slug} className={`rounded-2xl bg-white p-6 shadow-sm ring-1 ${pathway.cardRing}`}>
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <span className={`rounded-full px-3 py-1 text-xs font-semibold ring-1 ${pathway.badge}`}>Arts & Sports</span>
              ...
              <section>
                ...
                <ul className="mt-2 space-y-2 text-sm text-slate-700">
                  {subject.competencyIndicators.map((indicator) => (
                    <li key={indicator} className="rounded-lg bg-orange-50 px-3 py-2 ring-1 ring-orange-100">{indicator}</li>
                  ))}
                </ul>
              </section>
              <section>
                ...
                <ul className="mt-2 space-y-2 text-sm text-slate-700">
                  {subject.learningOutcomes.map((outcome) => (
                    <li key={outcome} className="rounded-lg bg-orange-50 px-3 py-2 ring-1 ring-orange-100">{outcome}</li>
                  ))}
                </ul>
              </section>
            </div>
